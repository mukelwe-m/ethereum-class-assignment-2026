...
contract OrderBook {
    ...
    function placeBuyOrder(uint256 amount, uint256 price) external returns (uint256 orderId) {
        ...
        IERC20(FNBT).safeTransferFrom(msg.sender, address(this), totalCost);
        ...
        emit OrderPlaced(orderId, msg.sender, 0, FNBT, PNPT, amount, price);
    }
    ...
    function placeSellOrder(uint256 amount, uint256 price) external returns (uint256 orderId) {
        ...
        IERC20(PNPT).safeTransferFrom(msg.sender, address(this), amount);
        ...
        emit OrderPlaced(orderId, msg.sender, 1, PNPT, FNBT, amount, price);
    }
